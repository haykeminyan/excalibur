...
function updateItem(pk) {
  const updateUrl = `/facture/world/update/${pk}/`; // Construct URL dynamically
  window.location.href = updateUrl; // Redirect to the update view
}

function deleteItem(pk) {
  const deleteUrl = `/facture/world/delete/${pk}/`; // Construct URL dynamically
    ...
    fetch(deleteUrl, {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          window.location.href = '/facture/world/';
        } else {
